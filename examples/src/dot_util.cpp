...
void sleep_wake_interrupt_only(bool deepsleep)
{
    ...
    logInfo("%ssleeping until interrupt on %s pin", deepsleep ? "deep" : "", deepsleep ? "DIO2" : mDot::pinName2Str(dot->getWakePin()).c_str());
...
void sleep_wake_rtc_or_interrupt(bool deepsleep)
{
    ...
    logInfo("%ssleeping %lus or until interrupt on %s pin", deepsleep ? "deep" : "", delay_s, deepsleep ? "DIO2" : mDot::pinName2Str(dot->getWakePin()).c_str());
...
int recv_data(std::vector<uint8_t> data)
{
    ...
    ret_recv = dot->recv(data);
    if ((ret_recv = dot->recv(data)) != mDot::MDOT_OK) {
        //logError("failed to recv: [%d][%s]", ret_recv, mDot::getReturnCodeString(ret_recv).c_str());
    } else {
		
        logInfo("Llego un Dato: %s");
    }
